...
def initialize_tts(tts_engine, model_name):
    # This function will check for CUDA availability and initialize the TTS engine accordingly
    device = "cuda" if torch.cuda.is_available() else "cpu"
    if tts_engine == 'coqui':
        # Initialize TTS with GPU support if available
        tts = TTS(model_name=model_name, progress_bar=False).to(device)
    elif tts_engine == 'gtts':
        # gTTS does not use PyTorch and does not need GPU support
        tts = gTTS(text='', lang='en')
    else:
        # pyttsx3 does not use PyTorch and does not need GPU support
        tts = pyttsx3.init()
        voices = tts.getProperty('voices')
        tts.setProperty('voice', voices[1].id)  # setting to female voice
...
tts = initialize_tts(ttsengine, "tts_models/en/ljspeech/vits--neon")
